# LicenseManager — Local-build specs

> This fork replaces remote licensing with a permanent local entitlement. Historical upstream
> trial and activation behavior below is retained only as background and is not wired at runtime.

The local contract is:

- `state` is always `.pro`;
- `isProAvailable` is always `true` and `isProLocked` is always `false`;
- activation, deactivation, and revalidation perform no network or Keychain I/O;
- all completion handlers resolve locally;
- debug trial helpers cannot put the application into a locked state.
